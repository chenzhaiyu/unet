...
def testGenerator(test_path, target_size=(512, 512, 3), flag_multi_class=False, as_gray=True):
    all_test_image_names = os.listdir(test_path)
    for image_name in all_test_image_names:
...
def saveResult(save_path, npyfile, flag_multi_class=False, num_class=2):
    for i, item in enumerate(npyfile):
        ...
        output = (img > 0.5).astype(np.uint8) * 255
        io.imsave(os.path.join(save_path, "%d_predict.tif" % i), output)
